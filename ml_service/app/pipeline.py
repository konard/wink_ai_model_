...
VIOLENCE_WORDS = [
    ...
    r'\battack(ed|ing)?\b', r'\bbeat(s|en|ing)?\b',
    r'\bthug(s)?\b', r'\bterror\b', r'\bfight(ing)?\b',
    r'\bbattle(s|d)?\b', r'\bwar\b', r'\bshoot[- ]?out\b',
    r'\bexplosion\b', r'\bgrenade\b', r'\bcorps(e|es)?\b'
]
...
class RatingPipeline:
    ...
    def scene_feature_vector(self, scene_text: str) -> Dict[str, float]:
        ...
        violence = self.count_matches([re.compile(p, re.I) for p in VIOLENCE_WORDS], txt)
        ...
        child_mentions = self.count_matches([re.compile(p, re.I) for p in CHILD_PATTERN], txt)
